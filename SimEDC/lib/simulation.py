...
class Simulation:
    ...
    def __init__(self, mission_time,
        ...
        if self.use_power_outage:
            self.racks = [Rack(None, None) for i in range(num_racks)]
        else:
            self.racks = [Rack(self.rack_fail_dists, self.rack_repair_dists) for i in range(self.num_racks)]

        if not use_trace:
            self.nodes = [Node(self.node_fail_dists, self.node_transient_fail_dists ,
                               self.node_transient_repair_dists) for i in range(self.num_nodes)]
        else:
            # It will generate in reset() of Regular Simualtion
            self.nodes = [Node(None, None, None) for i in range(self.num_nodes)]

        self.disks = [Disk(self.disk_fail_dists, None) for i in range(self.num_disks)]
