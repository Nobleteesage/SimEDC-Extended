...
class State:
    ...
    def __init__(self, num_disks=0, num_nodes=0):
        ...
        if self.num_disks != 0 and self.num_nodes != 0:
            # FIX 1: use integer division (//) instead of float division (/)
    ...
    def update_state_unifbfb(self, event_type, subsystem_idx):
        ...
        if event_type == Disk.EVENT_DISK_FAIL:
            ...
            # FIX 2: use integer division (//) so node_idx is always an int
            node_idx = subsystem_idx // self.disks_per_node
            node_fail = True
            for i in range(int(self.disks_per_node)):
            ...
            # FIX 3: use integer division (//) so node_idx is always an int
            node_idx = subsystem_idx // self.disks_per_node
            node_repair = True
            for i in range(int(self.disks_per_node)):
                disk_idx = node_idx * self.disks_per_node + i
            ...
            # FIX 4: wrap in int() to ensure range() gets an integer
            for i in range(int(self.disks_per_node)):
            ...
            # FIX 4 continued: same fix for repair loop
            for i in range(int(self.disks_per_node)):
                disk_idx = subsystem_idx * self.disks_per_node + i
